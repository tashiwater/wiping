...
class DCNNAE(chainer.Chain):
	...
	def encode(self, x, test):
		hid = F.tanh(self.bn1(self.conv1(x), test=test))
		hid = F.tanh(self.bn2(self.conv2(hid), test=test))
		hid = F.tanh(self.bn3(self.conv3(hid), test=test))
		hid = F.tanh(self.bn4(self.conv4(hid), test=test))
		hid = F.tanh(self.bn5(self.l5(hid), test=test))
		hid = F.tanh(self.bn6(self.l6(hid), test=test)) # 30dim

		return hid


	def decode(self, x, test):
		hid = F.tanh(self.bn7(self.l7(x), test=test))
		hid = F.reshape(F.tanh(self.bn8(self.l8(hid), test=test)), (x.data.shape[0], 256, 3, 4))
		hid = F.tanh(self.bn9(self.dconv9(hid), test=test))
		hid = F.tanh(self.bn10(self.dconv10(hid), test=test))
		hid = F.tanh(self.bn11(self.dconv11(hid), test=test))
		hid = F.tanh(self.dconv12(hid))
