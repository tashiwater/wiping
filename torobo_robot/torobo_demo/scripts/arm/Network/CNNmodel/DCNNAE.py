...
class DCNNAE(chainer.Chain):
	...
	def encode(self, x, test):
		...
		hid = F.sigmoid(self.bn6(self.l6(hid), test=test)) # 30dim
	...
	def decode(self, x, test):
		...
		hid = F.reshape(F.relu(self.bn8(self.l8(hid), test=test)), (x.data.shape[0], 256, 6, 8))
