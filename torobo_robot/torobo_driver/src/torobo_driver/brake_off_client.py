...
class BrakeOffClient(object):
    def __init__(self, ns):
        if (ns[-1] != "/"):
            ns += "/"
        self._ns = ns
        self.service = rospy.ServiceProxy(self._ns + 'brake_off', BrakeOff)

    def call_service(self, joint_names):
        rospy.loginfo('brake_off service client is called')
        ...
        try:
            ...
            if response.success:
                ret = "succeeded"
            rospy.loginfo('brake_off [%s] is %s' % (joint_names, ret))
